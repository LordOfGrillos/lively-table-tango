...
export function ReportCharts({ orders, className }: ReportChartsProps) {
  ...
  const salesByDayData = [
    { name: "Lun", value: 12500 },
    { name: "Mar", value: 15000 },
    { name: "Mié", value: 13200 },
    { name: "Jue", value: 16800 },
    { name: "Vie", value: 22000 },
  ...
  return (
    <div className={cn(className)}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Main Chart */}
        <Card className="lg:col-span-2 shadow-sm hover:shadow-md transition-shadow border-purple-100">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg font-medium text-gray-800">
              {activeChart === "sales" && "Ventas por Día"}
              {activeChart === "channels" && "Ventas por Canal de Distribución"}
              {activeChart === "orderTypes" && "Ventas por Tipo de Orden"}
            </CardTitle>
          </CardHeader>
        ...
        <Card className="shadow-sm hover:shadow-md transition-shadow border-purple-100">
          <CardHeader className="pb-2">
            <CardTitle className="text-lg font-medium text-gray-800">Distribución de Ventas</CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="h-[300px] flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={activeChart === "channels" ? salesByChannelData : salesByOrderTypeData}
                    cx="50%"
                    cy="50%"
                    labelLine={false}
                    outerRadius={80}
                  ...
                  >
                    {(activeChart === "channels" ? salesByChannelData : salesByOrderTypeData).map(
                      (entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={entry.color}
